      ...
      <section>
        ...
        >
          ...
          <div class="overflow-x-auto">
            ...
            >
              ...
              >
                <tr>
                  ...
                  <th scope="col" class="px-4 py-4">#OrderId</th>
              ...
              <tbody>
                ...
                <tr
                  v-else-if="
                    !orderStore.isLoading && orderStore.allOrder.length === 0
                  "
                >
                  <td
                    colspan="9"
                ...
                <tr
                  ...
                  v-for="(order, index) in orderStore.allOrder"
                  :key="order.id"
                >
                  <td class="px-4 py-3">
                    {{ (orderStore.currentPage - 1) * 10 + index + 1 }}
                  </td>
                  ...
                  <td class="px-4 py-3 text-start">
                    ...
                    <div
                      ...
                      class="z-50 absolute bg-white rounded divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600"
                    >
                  ...
                  <td class="px-4 py-3">{{ order.customer.email ?? "N/A" }}</td>
                  <td class="px-4 py-3">{{ order.customer.phone ?? "N/A" }}</td>
                  <td class="px-4 py-3">
                    $ {{ order.total_amount.toFixed(2) }}
                  </td>
                  <td class="px-4 py-3">
                    <span
                      :class="`text-xs font-medium me-2 px-2.5 py-0.5 rounded  ${getStatusClass(
                        order.status
                      )}`"
                    >
            ...
            <!-- pagination -->
            <nav
              v-if="orderStore.allOrder.length > 0"
            ...
            >
              ...
              >
                ...
                <span class="font-semibold text-gray-900 dark:text-white">
                  {{
                    orderStore.allOrder.length === 0
                      ? 0
                      : (orderStore.currentPage - 1) * 10 + 1
                  }}-{{
                    Math.min(
                      orderStore.currentPage * 10,
                      orderStore.allOrder.length
                    )
                  }}
                </span>
                of
              ...
              <ul class="inline-flex items-stretch -space-x-px">
                ...
                <li>
                  <a
                    @click.prevent="
                      orderStore.goToPage(orderStore.currentPage - 1)
                    "
                ...
                <!-- Page Number Buttons -->
                <li v-for="page in orderStore.totalPages" :key="page">
                  <a
                    @click.prevent="orderStore.goToPage(page)"
                    :class="{
                      'bg-blue-700 text-white': page === orderStore.currentPage,
                      'text-gray-500 bg-white': page !== orderStore.currentPage,
                    }"
...
import { DotIcon, EditIcon, DeleteIcon, Loading, Eye } from "../../assets";
import { formatDate, getStatusClass, getStatusText } from "../../utilities";
...
const uploadFile = async () => {
  ...
  try {
    ...
    try {
      ...
      console.error("Error fetching order after upload:", fetchError);
    } finally {
      loading.value = false;
      file.value = null;
...
const downloadCsv = async () => {
  ...
  if (orderStore.allOrder.length === 0) {
  ...
  try {
    const response = await apiClient.get("/api/export-orders", {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
